...
import { updatePairDayData, updateTokenDayData, updateSphynxDayData, updatePairHourData } from "./dayUpdates";
...
export function handleSync(event: Sync): void {
  ...
  let sphynx = SphynxFactory.load(FACTORY_ADDRESS);

  // reset factory liquidity by subtracting only tracked liquidity
  sphynx.totalLiquidityBNB = sphynx.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal);
  ...
  sphynx.totalLiquidityBNB = sphynx.totalLiquidityBNB.plus(trackedLiquidityBNB);
  sphynx.totalLiquidityUSD = sphynx.totalLiquidityBNB.times(bundle.bnbPrice);
...
export function handleMint(event: Mint): void {
  ...
  sphynx.totalTransactions = sphynx.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let sphynx = SphynxFactory.load(FACTORY_ADDRESS);
  sphynx.totalVolumeUSD = sphynx.totalVolumeUSD.plus(trackedAmountUSD);
  sphynx.totalVolumeBNB = sphynx.totalVolumeBNB.plus(trackedAmountBNB);
  sphynx.untrackedVolumeUSD = sphynx.untrackedVolumeUSD.plus(derivedAmountUSD);
  sphynx.totalTransactions = sphynx.totalTransactions.plus(ONE_BI);
  ...
  let sphynxDayData = updateSphynxDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  sphynxDayData.dailyVolumeUSD = sphynxDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  sphynxDayData.dailyVolumeBNB = sphynxDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  sphynxDayData.dailyVolumeUntracked = sphynxDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  sphynxDayData.save();
